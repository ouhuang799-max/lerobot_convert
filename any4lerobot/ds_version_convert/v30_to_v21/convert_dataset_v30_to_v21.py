...
def convert_dataset(
    ...
    # Check if root already contains v2.1 data
    try:
        info = load_info(root)
        if info.get("codebase_version") == V21:
            raise ValueError(
                f"Root directory '{root}' already contains v2.1 data. "
                f"This script converts v3.0 -> v2.1. "
                f"If you want to convert v3.0 data, ensure the root contains v3.0 data. "
                f"Current v2.1 data is at: {root}"
            )
    except FileNotFoundError:
        # info.json doesn't exist, assume it's v3.0 or needs conversion
        pass

    old_root = root.parent / f"{root.name}_{V30}"
    new_root = root.parent / f"{root.name}_{V21}"

    # Backup existing v3.0 data if old_root exists
    if old_root.is_dir():
        logging.info(f"Removing existing backup directory: {old_root}")
        shutil.rmtree(old_root)
    
    # Backup existing v2.1 data before deletion
    if new_root.is_dir():
        timestamp = int(time.time())
        backup_path = root.parent / f"{root.name}_{V21}_backup_{timestamp}"
        logging.warning(
            f"Existing v2.1 data found at {new_root}. "
            f"Backing up to {backup_path} before conversion."
        )
        shutil.move(str(new_root), str(backup_path))
